...
const TAGS_CACHE_FILE = path.join(process.cwd(), "output", ".eslint-tags-cache.json");
...
export default createRule({
  ...
  meta: {
    ...
    messages: {
      duplicateTag: "Tag '{{tag}}' is duplicated. First occurrence in '{{file}}'.",
      duplicateTagInFile: "Tag '{{tag}}' is already used in this file at line {{line}}."
    },
    schema: []
  },
      ...
      CallExpression(node) {
        ...
        for (const rawTag of extractedTags) {
          ...
          if (tagPrefixes.length) {
            const hasValidPrefix = tagPrefixes.some((prefix) => tag.startsWith(prefix.toUpperCase()));
...
function isTestFunction(node) {
  ...
  if (node.callee.type === "MemberExpression") {
    ...
    if (object?.type === "Identifier" && object.name === "test" && property?.type === "Identifier") {
