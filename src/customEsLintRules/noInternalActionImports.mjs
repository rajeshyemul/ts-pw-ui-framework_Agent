...
function importsInternalActionModule(source, filePath) {
  if (source === "@helper/actions/internal" || source.startsWith("@helper/actions/internal/")) {
    return true;
  }

  if (source === "src/helper/actions/internal" || source.startsWith("src/helper/actions/internal/")) {
    return true;
  }
  ...
  const resolvedImportPath = normalizePath(path.resolve(path.dirname(filePath), source));
...
export default createRule({
  ...
  meta: {
    ...
    docs: {
      description: "Disallow importing internal UI action classes outside the action facade.",
      recommended: true
    },

    messages: {
      internalActionImport: "Use UIActions instead of importing internal action classes directly."
    },
