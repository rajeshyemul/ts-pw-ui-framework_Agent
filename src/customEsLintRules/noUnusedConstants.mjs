...
export default createRule({
  ...
  meta: {
    ...
    messages: {
      unusedConstant: "Constant '{{constantName}}' in class '{{className}}' is not used anywhere inside src.",
      unusedLocator: "Locator '{{locatorName}}' in class '{{className}}' is not used anywhere inside src."
    }
  },
      ...
      "Program:exit"() {
        ...
        for (const property of properties) {
          ...
          for (const file of allFiles) {
            const usages = scanFileForUsage(file, property.className, property.name, currentFileName);
          ...
          const actualUsages = totalUsages.filter((usage) => {
            const isSelfDeclaration = usage.file === relativeFilePath && usage.line === property.line;
          ...
          if (actualUsages.length === 0) {
            const messageType = isConstantsFile ? "unusedConstant" : "unusedLocator";
